...
/** -----------------------------------------------------------------------------*/
 ...
 /** -----------------------------------------------------------------------------*/
  ...
  /** Here is a function that makes a map for searching. */
  void searchh(){
    double o = 6.5;
    double s = 5.0;
 ...
 //* Here we call searchh function and create a first searching map.  */
  searchh();
 ...
 /* Here we initialize arrays of lidar layers.*/
 //lidar1
  double layer_1[20];
  double layer_2[20];
  double layer_3[20];
 ...
 //object detection starts here. 
    ...
    /* Here we take a current image of every layer.*/
    ...
    const float *layer41 = wb_lidar_get_layer_range_image(lidar2, 0);
    const float *layer42 = wb_lidar_get_layer_range_image(lidar2, 1);
    const float *layer43 = wb_lidar_get_layer_range_image(lidar2, 2);
    const float *layer44 = wb_lidar_get_layer_range_image(lidar2, 3);
    const float *layer45 = wb_lidar_get_layer_range_image(lidar2, 4);
    const float *layer46 = wb_lidar_get_layer_range_image(lidar2, 5);
    const float *layer47 = wb_lidar_get_layer_range_image(lidar2, 6);
    const float *layer48 = wb_lidar_get_layer_range_image(lidar2, 7);
    const float *layer49 = wb_lidar_get_layer_range_image(lidar2, 8);
    const float *layer410 = wb_lidar_get_layer_range_image(lidar2, 9);
    const float *layer411 = wb_lidar_get_layer_range_image(lidar2, 10);
    const float *layer412 = wb_lidar_get_layer_range_image(lidar2, 11);
    const float *layer413 = wb_lidar_get_layer_range_image(lidar2, 12);
    const float *layer414 = wb_lidar_get_layer_range_image(lidar2, 13);
    const float *layer415 = wb_lidar_get_layer_range_image(lidar2, 14);
    const float *layer416 = wb_lidar_get_layer_range_image(lidar2, 15);
    ...
    /*Here we take a value of distancesensor.*/  
    ...
    /*We want to know when lidar sensors are in use.  */
    if (lidarsensors == false){
      printf("\n");
      printf("lidarsensors disabled ");
      printf("\n");
    }
    ...
    if (lidarsensors == true){
      ...
      /*Here we take a first value of every lidar layer. */
      if (d == 0){
        for ( int i = 0; i < 20; i++){
        ...
        for ( int i = 0; i < 20; i++){
          ...
          layer_4[i] = *(layer4+i);                      
              ...
              for ( int i = 0; i < 20; i++){
                ...
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer9 %f", *(layer49+i));
                  lay9[i] = *(layer49+i);
                }
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer10 %f", *(layer410+i));
                  lay10[i] = *(layer410+i);
                }
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer11 %f", *(layer411+i));
                  lay11[i] = *(layer411+i);
                }
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer12 %f", *(layer412+i));
                  lay12[i] = *(layer412+i);
                }
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer13 %f", *(layer413+i));
                  lay13[i] = *(layer413+i);
                }
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer14 %f", *(layer414+i));
                  lay14[i] = *(layer414+i);
                }
                 printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer15 %f", *(layer415+i));
                  lay15[i] = *(layer415+i);
                }
                printf("\n");
                for ( int i = 0; i < 20; i++){
                  printf("lidar2 layer16 %f", *(layer416+i));
                  lay16[i] = *(layer416+i);
                }
      ...
      /*Here we compare current lidar values to the earlier values and
       if there is a big enough difference, we can assume that there is
        a object under the drone. */
        
      /*After recognition drone uses those lidar sensors to move backwards
       until it is on top of the object.*/
        for ( int i = 0; i < 20; i++){
        ...
        /*Here we check lidar layers which are set horizontally. 
        Based on value differences, drone moves left or right until
         it’s on top of the object.*/ 
        if ( over == true ){
                for ( int i = 0; i < 20; i++){     
                ...
                printf("\n");
                for ( int i = 0; i < 20; i++){                     
                      float m = lay9[i] - *(layer49+i);                     
                      if (m > 0.03 || m < -0.03){
                        if (s == true){
                          leftright = 0.065;
                          printf("lidar2 layer9 %f", m);
                        }else{
                          leftright = 0.1;
                          printf("lidar2 layer9 %f", m);
                        }
               ...
               printf("\n");
                for ( int i = 0; i < 20; i++){                     
                      float m = lay10[i] - *(layer410+i);                      
                      if (m > 0.03 || m < -0.03){
                        leftright = 0.2;
                        
                        printf("lidar2 layer10 %f", m);                        
                      }
                }
                
                printf("\n");
                for ( int i = 0; i < 20; i++){     
                        float m = lay12[i] - *(layer412+i);                      
                        if (m > 0.03 || m < -0.03){
                          leftright = 0.4;
                          printf("lidar2 layer12 %f", m);
                        }
                }
                
                 printf("\n");
                for ( int i = 0; i < 20; i++){                  
                      float m = lay14[i] - *(layer414+i);                     
                      if (m > 0.03 || m < -0.03){
                        leftright = 0.4;
                        printf("lidar2 layer14 %f", m);
                      }
                }
              
                 printf("\n");
                  for ( int i = 0; i < 20; i++){                  
                      float m = lay15[i] - *(layer415+i);  
                                          
                      if (m > 0.03 || m < -0.03){
                        leftright = 0.4;
                        printf("lidar2 layer15 %f", m);
                      }
                }
                 printf("\n");
                for ( int i = 0; i < 20; i++){                  
                      float m = lay16[i] - *(layer416+i);                     
                      if (m > 0.03 || m < -0.03){
                        leftright = 0.4;
                        printf("lidar2 layer16 %f", m);
                      }
                }
      ...
      /** -----------------------------------------------------------------------------*/
        ...
        /*Here we check value of the distance sensor.
         Based on that value drone knows if there is an object under it or not.
         If there is object under the drone, it tries to pick it up. */
        if (distance_sensor_value < 780.0 && !detected) {
            ...
            if (distance_sensor_value < 200.0){
              target_altitude =0.2;
   ...
   /** -----------------------------------------------------------------------------*/
    ...
    /*Here we calculate a direction of the drone and 
    a distance from the target coordinates.
    */
    double e = pow(x2,2);
    double u = pow(y2,2);
    double x1 = x;
    ...
    if ( search == true){
      ...
      x1 = x - search1[n][0];
      y1 = y - search1[n][1];
      printf("target coordinate x:  %f\n", search1[n][0]);
      printf("target coordinate y:  %f\n", search1[n][1]);
      e = pow(x1,2);
      u = pow(y1,2);
    }
    double dy= y-y2;
    ...
    /* If search is true, we start searching. 
    This uses drones’ direction and distance to move it to the target coordinates.
    */
    ...
    /*If home is true, drone takes a direction to the home coordinates.*/
