...
int main(int argc, char **argv) {
  ...
  wb_position_sensor_enable(PS, 1);
  wb_position_sensor_enable(PS1, 1);
  wb_position_sensor_enable(PS2, 1);
  wb_position_sensor_enable(PS3, 1);
 ...
 //object detection starts here. 
    ...
    if (holding_object == true) {
        ...
        printf("PS1 val: %f \n", wb_position_sensor_get_value(PS1));
        printf("PS2 val: %f \n", wb_position_sensor_get_value(PS2));
        printf("PS3 val: %f \n", wb_position_sensor_get_value(PS3));
        
        if (wb_position_sensor_get_value(PS1) == 0) {
          wb_brake_set_damping_constant(brake, 1);
        } else {
          wb_brake_set_damping_constant(brake, 0);
        }
    ...
    //object detection ends here. 
        ...
        if(time - start_time >= 30 && !wait){
          lidarsensors = false;
          detected = false;
          
        }
        double distance_dif = 10 / (distdif - distance_sensor_value);
